...
const String endpoint = "http://dataservice.accuweather.com/currentconditions/v1/xxxxxxxxxxxxx.json?apikey=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx&language=es";
...
void setup() {
  ...
  //pinMode(39, INPUT); //boton entrada 39 
  M5.Axp.ScreenBreath(15);
  //pinMode(M5_BUTTON_HOME,INPUT_PULLUP); 
      pinMode(M5_BUTTON_RST, INPUT_PULLUP);
      pinMode(M5_BUTTON_HOME,INPUT_PULLUP); //Si se usa una resistencia pullup, el pin estará a nivel LOW cuando el interruptor esté cerrado.
...
/* if(digitalRead(M5_BUTTON_HOME) == HIGH){
    while(digitalRead(M5_BUTTON_HOME) == HIGH);
     Serial.println("M5 Button Home: pressed!");
      M5.Axp.ScreenBreath(7);
        
  } 
*/ 
//if (  digitalRead(M5_BUTTON_RST) == HIGH ) 

uint8_t led_count = 7;

   if(digitalRead(M5_BUTTON_HOME) == LOW){
      Serial.println("M5_BUTTON_HOME was pressed == TRUE");
   ...
   if(digitalRead(M5_BUTTON_RST) == LOW){
      Serial.println("M5_BUTTON_RST was pressed == TRUE");
